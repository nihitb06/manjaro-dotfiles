...
-- Enable VIM help for hotkeys widget when client with 
...
local theme_path = "~/.config/awesome/themes/machine/"

beautiful.init(theme_path .. "theme.lua")
beautiful.icon_theme        = "Papirus-Dark"
beautiful.bg_normal         = "#222D32"
beautiful.bg_focus          = "#2C3940"
beautiful.titlebar_close_button_normal = theme_path .. "titlebar/close_normal_adapta.png"
beautiful.titlebar_close_button_focus = theme_path .. "titlebar/close_focus_adapta.png"
...
awful.screen.connect_for_each_screen(function(s)
    ...
    s.mytasklist = awful.widget.tasklist(s, awful.widget.tasklist.filter.currenttags, tasklist_buttons, awful.placement.centered)
    ...
    s.mywibox:setup {
        ...
        { -- Right widgets
            layout = wibox.layout.fixed.horizontal,
            assault({
                -- battery ID to get data from
                battery = "BAT0", 
                -- ID of the AC adapter to get data from
                adapter = "AC", 
                -- width of battery
                width = 24, 
                -- height of battery
                height = 10, 
                -- width of charging bolt
                bolt_width = 12, 
                -- height of charging bolt
                bolt_height = 6, 
                -- width of battery border
                stroke_width = 2, 
                -- distance from the top of the battery to 
                -- the start of the peg
                peg_top = (calculated), 
                -- height of the peg
                peg_height = 5, 
                -- width of the peg
                ...
                -- font to use
                font = beautiful.font, 
                -- battery percentage to mark as critical 
                -- (between 0 and 1, default is 10%)
                critical_level = 0.15, 
                -- color to draw the battery when 
                -- it's discharging
                normal_color = beautiful.fg_normal, 
                -- color to draw the battery when it's at 
                -- critical level
                critical_color = "#CF6673", 
                -- color to draw the battery when it's 
                -- charging
                charging_color = "#009688" 
            }),
...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey, "Control"}, "Escape", 
        function () 
            awful.spawn.with_shell("sh ~/.config/rofi/rofi/launchers/launcher.sh") 
        end,
        {description = "launch rofi", group="Launcher"}),
    awful.key({ modkey }, "c", function () awful.spawn.with_shell("sh ~/.config/rofi/rofi/launchers/launcher-calc.sh") end,
              {description = "launch rofi calculator", group="Launcher"}),
    awful.key({ modkey, "Control"}, "f", 
        function () 
            awful.spawn.with_shell("sh ~/.config/rofi/rofi/launchers/launcher-find.sh") 
        end,
        {description = "launch rofi find", group="Launcher"}),
    awful.key({ modkey, "Control"}, "q", 
        function () 
            awful.spawn.with_shell("sh ~/.config/rofi/rofi/android/powermenu.sh") 
        end,
    ...
    awful.key({ }, "XF86AudioRaiseVolume", function()
        os.execute("amixer -q set Master 5%+ unmute")
        awful.spawn.with_shell(
            "sh ~/.config/rofi/rofi/bin/volume.sh"
        )
    end),
    ...
    awful.key({ }, "XF86MonBrightnessUp", function()
        ...
        awful.spawn.with_shell(
            "sh ~/.config/rofi/rofi/bin/backlight.sh"
        )
    end),
...
client.connect_signal("manage", function (c)
    ...
    -- Set rounded corners to clients
...
client.connect_signal("request::titlebars", function(c)
    ...
    if 
        not (l.name == "floating" or c.floating) 
        then 
